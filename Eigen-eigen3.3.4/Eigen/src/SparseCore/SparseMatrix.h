...
#ifdef USE_EIGEN_USERMALLOC

#include <Eigen_usermalloc.h>

#else//USE_EIGEN_USERMALLOC

#define MALLOC(SIZE)	std::malloc((SIZE))
#define FREE(PTR)		std::free(PTR)

#endif//USE_EIGEN_USERMALLOC
  ...
  protected:
    ...
    inline void reserveInnerVectors(const SizesType& reserveSizes)
    {
      if(isCompressed())
      {
        ...
        m_innerNonZeros = static_cast<StorageIndex*>(MALLOC(m_outerSize * sizeof(StorageIndex)));
      ...
      {
        StorageIndex* newOuterIndex = static_cast<StorageIndex*>(MALLOC((m_outerSize+1)*sizeof(StorageIndex)));
  ...
  public:
    ...
    void uncompress()
    {
      if(m_innerNonZeros != 0)
        return; 
      m_innerNonZeros = static_cast<StorageIndex*>(MALLOC(m_outerSize * sizeof(StorageIndex)));
      if (!m_innerNonZeros) { internal::throw_std_bad_alloc(); return; }
    ...
    void conservativeResize(Index rows, Index cols) 
    {
      ...
      else if (innerChange < 0) 
      {
        ...
        m_innerNonZeros = static_cast<StorageIndex*>(MALLOC((m_outerSize+outerChange+1) * sizeof(StorageIndex)));
        if (!m_innerNonZeros) internal::throw_std_bad_alloc();
        for(Index i = 0; i < m_outerSize; i++)
          m_innerNonZeros[i] = m_outerIndex[i+1] - m_outerIndex[i];
      }
    ...
    void resize(Index rows, Index cols)
    {
      ...
      {
        FREE(m_outerIndex);
        m_outerIndex = static_cast<StorageIndex*>(MALLOC((outerSize + 1) * sizeof(StorageIndex)));
